...
def create_dummy_video(path: Path):
    ...
    out = cv2.VideoWriter(
        str(temp_vid), cv2.VideoWriter_fourcc(*"mp4v"), 24, (320, 240)
    )
    for i in range(24):
...
def test_batch_folder_execution(tmp_path, monkeypatch, mock_store):
    ...
    config = Config(input_path=in_dir, output_path=out_dir, profile=HardwareProfile.CPU)
...
def test_checkpoint_crash_and_resume(tmp_path, mock_store):
    ...
    config = Config(
        input_path=in_file, output_path=out_file, profile=HardwareProfile.CPU
    )
