...
from pureframe.pipeline.shots import Shot, ShotVerdict, Category, Action, Box
...
def fuse(
    ...
    NUDITY_LABELS = {"FEMALE_GENITALIA_EXPOSED", "MALE_GENITALIA_EXPOSED", "FEMALE_BREAST_EXPOSED", "BUTTOCKS_EXPOSED", "ANUS_EXPOSED"}
    ...
    if scene_ctx.explicit_act_score >= (0.40 * t_mod) and audio_ctx.sexual_audio_score >= (0.30 * t_mod):
    ...
    if scene_ctx.implied_sex_score >= (0.45 * t_mod) and (audio_ctx.moaning_score >= (0.35 * t_mod) or audio_ctx.sexual_audio_score >= (0.30 * t_mod)):
        return ShotVerdict(
            shot_index=shot.index,
