        ...
        try:
            with Progress(
                ...
                for shot in shots:
                    ...
                    if not frames_list:
                        verdict = ShotVerdict(shot_index=shot.index, action=Action.NONE, category=Category.SAFE, confidence=1.0, reasoning="No frames")
                    ...
                    if verdict.action != Action.NONE:
                        if verdict.action == Action.BLACK_BOX:
                            if verdict.category in (Category.KISS_INTENSE, Category.KISS_LIGHT):
                                ...
                                verdict.boxes = [Box(x1=b[0], y1=b[1], x2=b[2], y2=b[3], frame_idx=f) for f, boxes in smooth_mouths.items() for b in boxes]
                            else:
                                ...
                                from pureframe.pipeline.shots import Box
                                verdict.boxes = [Box(x1=b[0], y1=b[1], x2=b[2], y2=b[3], frame_idx=f) for f, boxes in smooth_boxes.items() for b in boxes]
...
def process_file(config: Config):
    ...
    console.print(f"Flagged {sum(1 for v in plan.verdicts if v.action != Action.NONE)} shots for censoring.")
...
def plan_cmd(
    ...
    output: Path = typer.Option(None, "--output", "-o", help="Path to output censorplan JSON (defaults to input.censorplan.json)"),
    ...
    config = Config.from_cli(
        input_path=input,
        output_path=input.with_name(f"{input.stem}.pureframe{input.suffix}"), # Dummy output path for DB tracking
        profile=profile,
...
def apply_cmd(
    ...
    output: Path = typer.Option(None, "--output", "-o", help="Path to output video file"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging")
):
    ...
    if output:
        ...
        config_dict["output_path"] = input.with_name(f"{input.stem}.pureframe{input.suffix}")
        
    config = Config(**config_dict)
    
    console.print(f"[bold blue]PureFrame Apply[/bold blue]")
    console.print(f"Applying plan: {plan_path.name}")
    execute_render(plan, config)

@app.command("plan-edit")
def plan_edit_cmd(
    plan_path: Path = typer.Argument(..., exists=True, help="Path to censorplan JSON to edit")
):
    ...
    while True:
        ...
        try:
            ...
            if not retry:
                console.print("[yellow]Exiting without fixing plan. It might be broken.[/yellow]")
                break
